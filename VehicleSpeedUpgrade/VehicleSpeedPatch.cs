...
namespace AshFox.Subnautica
{
    ...
    internal static class CyclopsSpeedPatch
    {
        ...
        private static void ApplySpeedMultiplierWithEngineLevel(SubControl subControl)
        {
            ...
            // エネルギー消費処理を追加（移動中のみ）
            if (VehicleSpeedPatch.EnableEnergyConsumption && IsCyclopsMoving(subControl))
            {
                float batteryValue = VehicleSpeedPatch.CalculateCyclopsEnergyConsumption(
                    subControl.sub.upgradeConsole.modules,
                    currentEngineLevel
                );
                Plugin.Log.LogInfo($"Cyclops energy consumption: {batteryValue:F2}");
                if (batteryValue > 0)
                {
                    // CyclopsのPowerRelayを取得してエネルギー消費
                    Plugin.Log.LogInfo($"batteryValue is greater than 0");
                    var powerRelay = subControl.sub.GetComponent<PowerRelay>();
                    if (powerRelay != null)
                    {
                        // 消費前のバッテリー量を取得
                        float currentPower = powerRelay.GetPower();
                        float maxPower = powerRelay.GetMaxPower();
                        powerRelay.ConsumeEnergy(batteryValue, out float amountConsumed);
                        // 消費後のバッテリー量を取得
                        float newPower = powerRelay.GetPower();
                        if (VehicleSpeedPatch.WriteDebugLog)
                        {
                            Plugin.Log.LogInfo(
                                $"Cyclops battery: {newPower:F2}/{maxPower:F2}({newPower / maxPower:F2}%) (diff: {newPower - currentPower:F2}) (consumed: {amountConsumed:F2})"
                            );
                        }
                    }
                    else
                    {
                        Plugin.Log.LogInfo($"PowerRelay is null");
                    }
                }
            }
        ...
        // Cyclopsが移動中かどうかを判定
        private static bool IsCyclopsMoving(SubControl subControl)
        {
            if (subControl?.sub?.GetComponent<Rigidbody>() == null)
            {
                Plugin.Log.LogWarning("Cyclops Rigidbody not found");
                return false;
            }

            var rigidbody = subControl.sub.GetComponent<Rigidbody>();
            ...
            // 移動中かどうかを判定
            bool isMoving = currentVelocity > velocityThreshold;
            if (VehicleSpeedPatch.WriteDebugLog)
            {
                Plugin.Log.LogInfo($"Cyclops velocity: {currentVelocity:F2}");
                Plugin.Log.LogInfo($"Cyclops moving status: {isMoving}");
            }
            return isMoving;
        }
    }
    ...
    internal static class VehicleEnergyConsumptionPatch
    {
        ...
        {
            if (!VehicleSpeedPatch.EnableEnergyConsumption)
            {
                return;
            }
            ...
            if (batteryValue > 0)
            {
                __instance.ModifyCharge(-batteryValue);
            }
            if (VehicleSpeedPatch.WriteDebugLog)
            {
                Plugin.Log.LogInfo(
                ...
                Plugin.Log.LogInfo(
                    $"Vehicle energy consumption increased: {amount:F2} (batteryValue: x{batteryValue:F2})"
                );
            }
        }
    }
